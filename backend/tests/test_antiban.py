...
async def test_antiban_working_hours():
    ...
    config = AntibanConfig(working_hours_start=9, working_hours_end=17, respect_working_hours=True)
...
async def test_antiban_search_limits():
    config = AntibanConfig(max_searches_per_hour=2, respect_working_hours=False)
    system = AntibanSystem(config=config)
