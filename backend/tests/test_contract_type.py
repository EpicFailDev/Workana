...
def test_fixed_default():
    assert (
        detect_contract_type(title="Desenvolvimento de site", description="Escopo fechado")
        == "project_fixed"
    )
...
def test_hourly_text_signal():
    assert (
        detect_contract_type(title="Suporte mensal", description="Pagamento por hora") == "hourly"
    )
