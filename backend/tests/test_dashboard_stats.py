...
async def test_get_dashboard_stats_correct_mapping(monkeypatch):
    ...
    mock_responses = [
        MagicMock(
            scalar_one_or_none=MagicMock(return_value=None), scalar=MagicMock(return_value=None)
        ),  # 1. stats_today
    ...
    class FakeSession:
        async def execute(self, stmt):
            ...
            return MagicMock(
                scalar=MagicMock(return_value=0), scalar_one_or_none=MagicMock(return_value=None)
            )
