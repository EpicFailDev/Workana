...
def test_validate_metrics_semantics():
    ...
    # Perfil legítimo
    assert (
        validate_metrics_semantics(valid_metrics, "https://www.workana.com/freelancer/joao-s")
        is True
    )
    ...
    assert (
        validate_metrics_semantics(bad_name_metrics, "https://www.workana.com/freelancer/joao-s")
        is False
    )

    # Pouca evidência (falta country, skills, member_since, etc)
    insufficient_metrics = {"display_name": "João S.", "username": "joao-s"}
    assert (
        validate_metrics_semantics(
            insufficient_metrics, "https://www.workana.com/freelancer/joao-s"
        )
        is False
    )
...
async def test_validation_then_sync_uses_cache():
    ...
    with (
        patch.object(ProfileScraperService, "_parse_profile_html", return_value=mock_metrics),
...
def test_profile_validate_endpoint():
    ...
    with patch(
        "app.api.routers.profile.profile_scraper.fetch_public_profile", new_callable=AsyncMock
    ) as mock_fetch:
        mock_fetch.return_value = mock_metrics
        response = client.post(
            "/api/profile/validate?url=https://www.workana.com/freelancer/joao-s"
        )
        assert response.status_code == 200
        data = response.json()
