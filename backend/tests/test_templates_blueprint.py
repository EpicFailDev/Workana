...
def test_compile_blueprint_to_content():
    ...
    blueprint = [
        ...
        },
        {
            ...
            "mode": "instruction",
            "enabled": True,
            "content": "Adote um tom profissional.",
        },
    ...
    blueprint = [
        {
            ...
            "mode": "literal",
            "enabled": True,
            "content": "Texto Literal de abertura",
        },
        {
            "id": "2",
            "type": "solucao",
...
async def test_generate_proposal_with_blueprint(
    mock_get_template, mock_get_config, mock_gemini_factory
):
    # Mock API call
...
def test_pydantic_blueprint_validation():
    ...
    with pytest.raises(ValidationError) as excinfo:
        ProposalTemplateCreate(
            name="Test",
            blueprint=[TemplateBlock(id="b1", type="abertura", mode="literal", content="  ")],
        )
    ...
    with pytest.raises(ValidationError) as excinfo:
        ProposalTemplateCreate(
            ...
            blueprint=[
                TemplateBlock(
                    id="b1", type="abertura", mode="literal", enabled=False, content="Olá"
                )
            ],
        )
...
@patch("app.database.crud.get_automation_config")
@patch("app.database.crud.get_template")
async def test_generate_proposal_template_not_found(
    mock_get_template, mock_get_config, mock_gemini_factory
):
...
@patch("app.database.crud.get_automation_config")
@patch("app.database.crud.get_template")
async def test_generate_proposal_returns_template_id_used(
    mock_get_template, mock_get_config, mock_gemini_factory
):
    mock_gemini_factory.create.return_value.generate_content.return_value.text = '{"proposal": "Minha proposta", "suggested_price": "R$ 900", "justification": "justificativa"}'
    mock_get_config.return_value = {"gemini_api_key": "dummy_key", "user_full_name": "Bob"}
...
def test_api_list_templates_includes_system(
    mock_get_templates, mock_get_active_system_template, client
):
    ...
    sys_mock.blueprint = [
        {"id": "b1", "type": "abertura", "mode": "literal", "enabled": True, "content": "Olá"}
    ]
    sys_mock.version = 1
...
@patch("app.api.routers.automation.crud.get_active_system_template")
def test_api_duplicate_system_template(
    mock_get_active_system_template, mock_create_template, client
):
    # Mock active system template
    sys_mock = MagicMock()
    sys_mock.slug = "workana-consultivo"
