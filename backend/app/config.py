...
class Settings(BaseSettings):
    ...
    encryption_key: Optional[str] = Field(
        default=None, description="Chave de criptografia simétrica de 32 bytes"
    )
    ...
    def validate_all_configs(self) -> "Settings":
        ...
        if "sqlite" in self.database_url.lower():
            raise ValueError(
                "O banco SQLite foi removido do runtime. Configure uma URL PostgreSQL válida."
            )
        ...
        if is_prod:
            if not self.secret_key or self.secret_key == insecure_secret:
                raise ValueError(
                    "A SECRET_KEY deve ser explicitamente definida e segura em produção (debug=False)."
                )
            if not self.encryption_key or self.encryption_key == insecure_encrypt:
                raise ValueError(
                    "A ENCRYPTION_KEY deve ser explicitamente definida e segura em produção (debug=False)."
                )
        else:
    ...
    scraper_type: str = Field(
        default="fast", description="Tipo de scraper: 'parallel' (browser) ou 'fast' (http)"
    )

    # Proxy e Resolvedor de Captcha
    proxy_url: Optional[str] = Field(
        default=None, description="URL do proxy rotativo (ex: http://user:pass@host:port)"
    )
    captcha_provider: Optional[str] = Field(
        default=None, description="Provedor de captcha ('2captcha' ou 'anti-captcha')"
    )
    captcha_api_key: Optional[str] = Field(
        default=None, description="Chave de API do provedor de captcha"
    )
    ...
    log_level: str = Field(
        default="INFO", description="Nível mínimo dos logs (DEBUG/INFO/WARNING/ERROR/CRITICAL)"
    )
    ...
    slow_query_ms: int = Field(
        default=500, description="Limiar (ms) para logar queries SQL lentas, sem parâmetros"
    )
    ...
    workana_conversion_rate: float = Field(
        default=5.0, description="Taxa de conversão USD/BRL usada internamente pelo Workana"
    )
