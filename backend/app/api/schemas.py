...
class SearchFilters(BaseModel):
    ...
    pages_to_fetch: int = Field(
        default=10, ge=1, le=100, description="Número de páginas para buscar simultaneamente"
    )
...
class Project(BaseModel):
    ...
    client_projects_posted: Optional[int] = Field(
        None, description="Projetos publicados pelo cliente"
    )
...
def validate_blueprint_logic(v):
    ...
    for b in v:
        if b.enabled and (not b.content or not b.content.strip()):
            raise ValueError(
                f"O bloco com ID '{b.id}' e tipo '{b.type}' não pode ter conteúdo vazio."
            )
...
class ProposalTemplate(BaseModel):
    ...
    blueprint: List[TemplateBlock] = Field(
        default=[], description="Lista ordenada de peças do blueprint"
    )
...
class ProposalSaveRequest(BaseModel):
    ...
    proposal_id: Optional[int] = Field(
        None, description="ID da proposta para atualizar versão existente"
    )
...
class ProfileConfigUpdate(BaseModel):
    ...
    sync_interval_hours: int = Field(
        default=6, ge=1, le=24, description="Intervalo de sincronização em horas"
    )
