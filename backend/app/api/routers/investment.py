...
class InvestmentRequest(BaseModel):
    ...
    custom_percentages: Optional[Dict[str, float]] = Field(
        None, description="Optional custom percentages per stage title"
    )
...
async def calculate_investment(request: InvestmentRequest):
    ...
    try:
        breakdown = InvestmentBreakdownCalculator.get_breakdown_as_dict(
            total_value=request.total_value, custom_percentages=request.custom_percentages
        )
...
async def preview_investment(total_value: float):
    ...
    return {
        ...
        "stages_summary": [f"{s['title']}: {s['amount_formatted']}" for s in breakdown["stages"]],
    }
