    ...
    try:
        async with async_session() as session:
            ...
            config_result = await session.execute(
                select(ProfileConfig).where(ProfileConfig.user_id == user["user_id"])
            )
            ...
            metrics_result = await session.execute(
                ...
                .where(
                    and_(
                        ProfileMetrics.profile_url == config.profile_url,
                        ProfileMetrics.user_id == user["user_id"],
                    )
                )
        ...
        return ProfileMetricsResponse(success=False, error=f"Erro ao buscar métricas: {str(e)}")
...
async def sync_profile_metrics(force: bool = False, user: dict = Depends(get_current_user)):
    ...
    try:
        async with async_session() as session:
            ...
            logger.info(
                f"Iniciando sincronização do perfil para o usuário {user['user_id']}: {config.profile_url}"
            )

            # Fazer scraping do perfil público
            metrics_data = await profile_scraper.fetch_public_profile(
                config.profile_url, force_refresh=force
            )
            ...
            logger.info(
                f"Métricas do perfil sincronizadas com sucesso para o usuário {user['user_id']}"
            )
...
async def update_profile_config(
    config_update: ProfileConfigUpdate, user: dict = Depends(get_current_user)
):
    """
        ...
        async with async_session() as session:
            ...
            if config:
                ...
                if config.profile_url != config_update.profile_url:
                    ...
                    logger.info(
                        f"URL do perfil alterada para: {config_update.profile_url}. Resetando status de sincronização."
                    )
            ...
            logger.info(
                f"Configuração do perfil atualizada para o usuário {user['user_id']}: {config_update.profile_url}"
            )

            return MessageResponse(
                success=True, message="Configuração do perfil atualizada com sucesso!"
            )
...
async def get_profile_history(limit: int = 30, user: dict = Depends(get_current_user)):
    ...
    try:
        async with async_session() as session:
            ...
            return ProfileMetricsHistoryList(history=history, total=len(history))
