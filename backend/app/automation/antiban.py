...
class AntibanSystem:
    ...
    async def human_typing_delay(self):
        ...
        delay = random.randint(self.config.typing_delay_min_ms, self.config.typing_delay_max_ms)
        await asyncio.sleep(delay / 1000)
                ...
                try:
                    ...
                    if not stats:
                        ...
                        try:
                            ...
                            result = await session.execute(
                                select(AntibanStatsModel).where(
                                    AntibanStatsModel.user_id == user_id
                                )
                            )
                    ...
                    if attempt == max_retries - 1:
                        logger.error(
                            f"Erro de concorrência persistente ao atualizar AntibanStats para o usuário {user_id}"
                        )
                        raise
    ...
    async def can_send_proposal(self, user_id: Optional[Any] = None) -> tuple[bool, str]:
        ...
        if not self.is_within_working_hours():
            return (
                False,
                f"Fora do horário de operação ({self.config.working_hours_start}h-{self.config.working_hours_end}h)",
            )
        ...
        if stats.proposals_sent_this_hour >= self.config.max_proposals_per_hour:
            return (
                False,
                f"Limite horário atingido ({self.config.max_proposals_per_hour} propostas/hora)",
            )
        ...
        if stats.consecutive_proposals >= self.config.long_break_after_proposals:
            return (
                False,
                f"Pausa obrigatória após {self.config.long_break_after_proposals} propostas consecutivas",
            )
    ...
    async def can_search(self, user_id: Optional[Any] = None) -> tuple[bool, str]:
        ...
        if stats.searches_this_hour >= self.config.max_searches_per_hour:
            return (
                False,
                f"Limite de buscas por hora atingido ({self.config.max_searches_per_hour})",
            )
    ...
    async def register_proposal_sent(self, user_id: Optional[Any] = None):
        ...
        if user_id is None:
            ...
            logger.info(
                f"Proposta registrada em memória: {self.in_memory_stats.proposals_sent_today}/{self.config.max_proposals_per_day} hoje"
            )
        else:
    ...
    async def get_status(self, user_id: Optional[Any] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_proposal_time": stats.last_proposal_time.isoformat()
            if stats.last_proposal_time
            else None,
            "session_start": stats.session_start_time.isoformat()
