...
class WorkanaAutomation:
    ...
    async def login(self, user_id: str) -> bool:
        ...
        try:
            ...
            await session_manager.save_storage_state(user_id, state, account_email=creds.get("email"))
        ...
        try:
            ...
            while time.time() < deadline:
                ...
                try:
                    ...
                    return {"success": False, "message": "O navegador foi fechado antes de concluir o login."}
    ...
    async def submit_proposal(self, user_id: str, proposal_data) -> any:
        ...
        try:
            ...
            await page.goto(bid_direct_url, wait_until="domcontentloaded", timeout=settings.scraping_timeout)
            await asyncio.sleep(2)

            # Verificar se foi bloqueado por WAF/Cloudflare
            solver = CaptchaSolver()
            if await solver.is_blocked(page):
                logger.info("Desafio Cloudflare detectado ao acessar proposta. Tentando resolver...")
                solved = await solver.detect_and_solve(page)
                if not solved:
            ...
            if "/login" in curr_url or "/users/login" in curr_url or "/signup" in curr_url:
                raise Exception("Sua sessão do Workana está expirada ou desconectada. Acesse Configurações > Conta Workana para reconectar sua conta.")
            ...
            try:
                ...
                logger.warning("Seletor principal demorou a responder, prosseguindo com preenchimento resiliente...")
            ...
            if csrf_ok and csrf_ok.get("all_present"):
                logger.info(f"✅ Tokens CSRF/DCST validados: csrf_name={csrf_ok['csrf_name']}, dcst={csrf_ok['dcst'][:16]}...")
            else:
                logger.warning(f"⚠️ Tokens de segurança incompletos: csrf_value={csrf_ok.get('csrf_value') is not None}, "
            ...
            for editor_sel in ['.fr-element', 'div[contenteditable="true"]', '.ql-editor', '.note-editable']:
            ...
            for q in questions:
                try:
                    ...
                    if not curr_val or not curr_val.strip():
                        await q.fill("Tenho total disponibilidade e experiência comprovada para atender aos requisitos solicitados neste projeto com excelência e cumprimento rigoroso de prazos.")
            ...
            try:
                skill_checkboxes = await page.query_selector_all('input[type="checkbox"][name^="skill-"], input[type="checkbox"][name*="skill" i]')
                for scb in skill_checkboxes[:5]:
                    try:
                        ...
                        pass
                if skill_checkboxes:
                    logger.info(f"Habilidades vinculadas automaticamente no formulário ({len(skill_checkboxes)} disponíveis).")
            except Exception:
                pass
            ...
            if att_path:
                ...
                if os.path.exists(att_path):
                    ...
                    try:
                        file_input = await page.query_selector('input[type="file"], #AttachmentUpload input[type="file"]')
                        if file_input:
            ...
            try:
                ...
                if turnstile_el:
                    logger.info("Elemento Turnstile detectado no formulário, aguardando validação...")
                    ...
                    for attempt in range(3):  # 3 tentativas, timeout progressivo
                        try:
                            ...
                            logger.info(f"✅ Cloudflare Turnstile validado (tentativa {attempt + 1}).")
                            break
                        except Exception:
                            if attempt < 2:
                                logger.info(f"Turnstile ainda não resolvido (tentativa {attempt + 1}/3), aguardando...")
                                await asyncio.sleep(2)
                            else:
                                logger.warning("⚠️ Turnstile não resolvido após 3 tentativas. Prosseguindo mesmo assim — o submit pode falhar.")
            except Exception:
                pass
                    ...
                    try:
                        ...
                        if file_input:
                            ...
                            if upload_btn:
                                ...
                                logger.info(f"📎 Arquivo anexado via file chooser: {os.path.basename(attachment)}")
                                await asyncio.sleep(5)
                            else:
                                logger.warning("⚠️ Input de arquivo não encontrado no formulário. Anexo ignorado.")
                    except Exception as e:
                        logger.warning(f"Não foi possível anexar arquivo: {e}. Prosseguindo sem anexo.")
                else:
            ...
            for extra_sel in [
                ...
                try:
                    ...
                    if skip_extras and await skip_extras.is_visible():
                        logger.info(f"Modal Super Bids detectado ({extra_sel}). Dispensando extras...")
            ...
            if redirect_has_added and redirect_has_bid:
                ...
                error_banner = await page.query_selector('.alert-danger, .error-message, .alert-error, .help-block.error, .form-error')
                if error_banner and await error_banner.is_visible():
                    ...
                    if err_text and len(err_text) > 3 and "sucesso" not in err_text.lower():
                        logger.warning(f"Alerta de erro detectado no Workana após submissão: {err_text}")
                ...
                if "/messages/bid/" in final_url:
                    logger.warning(f"⚠️ URL permaneceu na página de proposta após submit ({final_url[:100]}). O envio pode ter falhado silenciosamente.")
