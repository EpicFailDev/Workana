...
def validate_metrics_semantics(metrics: dict, final_url: str) -> bool:
    ...
    if evidence_count < 2:
        logger.error(
            f"Validação semântica falhou: poucas evidências ({evidence_count}) para o perfil"
        )
        return False
...
class ProfileScraperService:
    ...
    async def fetch_public_profile(
        cls, profile_url: str, force_refresh: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            async with httpx.AsyncClient(**client_kwargs) as client:
                ...
                if not metrics.get("display_name") or not validate_metrics_semantics(
                    metrics, profile_url
                ):
                    logger.info(
                        "Dados incompletos, inválidos ou zerados via HTTP, tentando com Playwright..."
                    )
                ...
                if metrics.get("success", False):
                    ...
                    logger.info(
                        f"Perfil coletado com sucesso: {metrics.get('display_name', 'Unknown')}"
                    )
                return metrics
            ...
            try:
                ...
                return {
                    "error": f"HTTP {e.response.status_code} / Playwright: {str(pe)}",
                    "success": False,
                }
            ...
            try:
                ...
                return {
                    "error": f"Erro principal: {str(e)} / Playwright: {str(pe)}",
                    "success": False,
                }
        ...
        async with cls._semaphore:
            ...
            try:
                ...
                async def block_resources(route):
                    ...
                    if req.resource_type in blocked_types or any(
                        p in url for p in blocked_patterns
                    ):
                        await route.abort()
                    else:
                ...
                if "/login" in final_url.lower():
                    raise Exception(
                        "Redirecionado para página de login (perfil privado ou inexistente)"
                    )
                ...
                try:
                    ...
                    logger.error(
                        f"Erro ao carregar página do perfil (elementos esperados não encontrados): {e}"
                    )
                ...
                if (
                    "não encontramos a página" in body_text.lower()
                    or "página não encontrada" in body_text.lower()
                    or "404" in body_text.lower()
                ):
                ...
                if not validate_metrics_semantics(metrics, final_url):
                    raise Exception(
                        "Falha na validação semântica dos dados coletados pelo Playwright"
                    )
    ...
    def _parse_profile_html(cls, html: str, profile_url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if title_text:
                ...
                clean_title = re.sub(
                    r"^(Workana|Freelancer|HERO)\.?\s*", "", clean_title, flags=re.IGNORECASE
                )
            ...
            if not candidate_name:
                ...
                if h1:
                    ...
                    h1_text = re.sub(
                        r"^(Workana|Freelancer|HERO)\.?\s*", "", h1_text, flags=re.IGNORECASE
                    )
            ...
            if not candidate_name:
                name_match = re.search(
                    r"(?:HERO|PLATINUM|GOLD|OURO|SILVER|PRATA|BRONZE|IRON|FERRO)\s+([A-Z][a-z]+\s+[A-Z]\.)",
                    body_text,
                    re.IGNORECASE,
                )
                if name_match:
            ...
            photo_img = soup.select_one(
                "img.img-circle, .profile-photo img, .avatar img, .profile-view img"
            )
            if photo_img:
            ...
            # === PAÍS e LOCALIZAÇÃO ===
            country_link = soup.select_one(
                ".profile-header a[href*='/freelancers/'], .profile-view a[href*='/freelancers/']"
            )
            if country_link:
                metrics["country"] = country_link.get_text(strip=True)
            else:
                loc_match = re.search(r"📍\s*([^\n]{2,30})", body_text) or re.search(
                    r"(Brasil|Argentina|Japão|México|Portugal|Espanha|United States|Japan)",
                    body_text,
                    re.IGNORECASE,
                )
                if loc_match:
            ...
            if p_val:
                metrics["projects_completed"] = int(p_val)

            o_val = get_metric(r"(?:Projetos em execução|Ongoing projects)[\s\n]*(\d+)")
            if o_val:
                metrics["projects_in_progress"] = int(o_val)

            h_val = get_metric(r"(?:Horas trabalhadas|Hours worked)[\s\n]*(\d+)")
            if h_val:
                metrics["hours_worked"] = int(h_val)

            reviews_match = re.search(
                r"(?:Classificações dos clientes|Ratings from clients)[\s\n]*\(?(\d+)\)?",
                body_text,
                re.IGNORECASE,
            ) or re.search(r"(\d+)\s*(reviews|classificaç)", body_text, re.IGNORECASE)
            if reviews_match:
                metrics["total_reviews"] = int(reviews_match.group(1))

            # Ingressou
            joined_match = re.search(
                r"(?:Ingressou|Joined|Member since)[\s\n]+([^\n]{3,30})", body_text, re.IGNORECASE
            )
            if joined_match:
            ...
            # === SKILLS ===
            skills = []
            skills_container = soup.select_one(
                ".skills, .profile-skills, .skills-list, [class*='skills']"
            )
            ...
            if skills_container:
                skill_elems = skills_container.select(
                    ".skill.label-info, .skill, .badge-skill, .tag, [href*='/skills/']"
                )
            else:
            ...
            login_pattern = re.compile(
                r"(há\s+\d+\s+(hora|minuto|dia)|last seen|online)", re.IGNORECASE
            )
