...
async def _sync_preferred_template(
    session: AsyncSession, user_id: Any, template_id: Optional[int]
) -> None:
...
async def get_preferred_or_default_template(user_id: Any) -> Optional[ProposalTemplateModel]:
    ...
    async with crud.async_session() as session:
        result = await session.execute(
            ...
            .where(
                and_(
                    ProposalTemplateModel.user_id == user_id,
                    ProposalTemplateModel.is_default == True,
                )
            )
            .limit(1)
        )
        ...
        if config and config.preferred_template_id:
            result = await session.execute(
                ...
                .where(
                    and_(
                        ProposalTemplateModel.id == config.preferred_template_id,
                        ProposalTemplateModel.user_id == user_id,
                    )
                )
                .limit(1)
...
async def get_active_system_template(
    slug: str = "workana-consultivo",
) -> Optional[SystemProposalTemplateModel]:
    ...
    async with crud.async_session() as session:
        result = await session.execute(
            ...
            .where(
                and_(
                    SystemProposalTemplateModel.slug == slug,
                    SystemProposalTemplateModel.is_active == True,
                )
            )
        ...
        fallback_content = ProposalPromptBuilder.compile_blueprint_to_content(
            DEFAULT_OFFICIAL_BLUEPRINT
        )
...
async def has_personal_default_or_preferred(user_id: Any) -> bool:
    ...
    async with crud.async_session() as session:
        ...
        result_config = await session.execute(
            select(AutomationConfigModel.preferred_template_id)
            .where(AutomationConfigModel.user_id == user_id)
            .limit(1)
        )
...
async def get_template(user_id: Any, template_id: int) -> Optional[ProposalTemplate]:
    ...
    async with crud.async_session() as session:
        result = await session.execute(
            select(ProposalTemplateModel).where(
                and_(
                    ProposalTemplateModel.id == template_id,
                    ProposalTemplateModel.user_id == user_id,
                )
            )
        )
...
async def create_template(user_id: Any, template: ProposalTemplateCreate) -> ProposalTemplate:
    ...
    if template.blueprint is not None:
        blueprint_data = [
            b.model_dump() if hasattr(b, "model_dump") else b.dict() for b in template.blueprint
        ]
...
async def update_template(
    user_id: Any, template_id: int, template: ProposalTemplateCreate
