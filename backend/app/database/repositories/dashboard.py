...
async def get_daily_stats(user_id: Any) -> Dict[str, int]:
    ...
    async with crud.async_session() as session:
        ...
        result = await session.execute(
            select(func.count(ProposalHistoryModel.id)).where(
                and_(
    ...
    async with crud.async_session() as session:
        ...
        favorites_res = await session.execute(
            select(func.count(ProjectModel.id)).where(
                and_(ProjectModel.is_favorite == True, ProjectModel.user_id == user_id)
            )
        )
        ...
        accepted_res = await session.execute(
            select(func.count(ProposalHistoryModel.id)).where(
                and_(
                    ProposalHistoryModel.status == "accepted",
                    ProposalHistoryModel.user_id == user_id,
                )
            )
        )
...
async def _update_daily_stats(
    user_id: Any, action_type: str, status: str, increment: int = 1
) -> None:
...
async def get_statistics(user_id: Any, days: int = 30) -> List[Dict[str, Any]]:
    ...
    async with crud.async_session() as session:
        ...
        result = await session.execute(
            ...
            .where(
                and_(
                    DailyStatisticsModel.date >= start_date, DailyStatisticsModel.user_id == user_id
                )
            )
    ...
    async with crud.async_session() as session:
        ...
        week_result = await session.execute(
            select(
                ...
                and_(DailyStatisticsModel.date >= week_ago, DailyStatisticsModel.user_id == user_id)
            )
        )
        ...
        month_result = await session.execute(
            select(
                ...
                and_(
                    DailyStatisticsModel.date >= month_ago, DailyStatisticsModel.user_id == user_id
                )
            )
        )
        ...
        return {
            ...
            "month": {"proposals_sent": month_stats[0] or 0, "projects_found": month_stats[1] or 0},
        }
