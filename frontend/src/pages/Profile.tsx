  ...
  if (!profileMetrics?.is_configured || isChangingProfile) {
    return (
      <div className="container py-8">
        <div
          className={`m3-card ${styles.notConfigured}`}
        ...
        >
          <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
            ...
            <h2 style={{ fontSize: '1.5rem', fontWeight: 600, marginBottom: '0.5rem' }}>
              Analisador de Perfil Workana
            </h2>
            <p style={{ color: 'var(--color-text-secondary)' }}>
              Cole a URL de um perfil público do Workana para visualizar métricas e histórico
              profissional.
            </p>
          </div>
          ...
          <div className="form-group" style={{ marginBottom: '1.5rem' }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              ...
              >
                {isValidatingProfile ? <span className="spinner spinner-sm"></span> : 'Verificar'}
              </button>
            </div>
          </div>
          ...
          {profileValidation && (
            <div
              className={`alert ${profileValidation.valid ? 'alert-success' : 'alert-error'}`}
            ...
            >
              {profileValidation.valid ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  ...
                  <span>
                    Perfil encontrado: <strong>{profileValidation.display_name}</strong>
                  </span>
                </div>
              ) : (
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem' }}>
            {isChangingProfile && profileMetrics?.is_configured && (
              <button className="btn btn-ghost" onClick={() => setIsChangingProfile(false)}>
                Cancelar
              </button>
            )}
            <button
              ...
              disabled={
                !newProfileUrl ||
                isSavingProfile ||
                (profileValidation ? !profileValidation.valid : false)
              }
          ...
          <div className={`m3-card ${styles.mainCard}`}>
            ...
            <div className={styles.photoContainer}>
              {profileMetrics.profile_photo_url ? (
                <img
                  src={profileMetrics.profile_photo_url}
                  alt={profileMetrics.display_name || ''}
                />
              ) : (
            ...
            <div className={styles.basicInfo}>
              ...
              {profileMetrics.profile_url && (
                <div style={{ marginTop: '1rem' }}>
                  <a
                    href={profileMetrics.profile_url}
                    target="_blank"
          ...
          <div className={`m3-card ${styles.statusCard}`}>
            ...
            <div className={styles.statusItem}>
              <span className={styles.statusValue}>
                {profileMetrics.last_sync
                  ? new Date(profileMetrics.last_sync).toLocaleString('pt-BR')
                  : 'Nunca'}
              </span>
