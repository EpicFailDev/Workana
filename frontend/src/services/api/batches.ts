...
export interface ProposalBatchItem {
  ...
  status:
    'queued' | 'generating' | 'ready' | 'sending' | 'sent' | 'failed' | 'skipped' | 'cancelled';
...
export const batchesApi = {
  ...
  async cancelProposalBatch(batchId: number) {
    return apiRequest<{ success: boolean; message: string }>(
      `/projects/batches/${batchId}/cancel`,
      {
        method: 'POST',
      }
    );
  ...
  async updateBatchItem(
    ...
    return apiRequest<{ success: boolean; item: ProposalBatchItem }>(
      `/projects/batches/items/${itemId}`,
      {
        method: 'PUT',
        body: data,
      }
  ...
  async sendBatchItemNow(itemId: number) {
    return apiRequest<{ success: boolean; message: string; proposal_id?: string }>(
      `/projects/batches/items/${itemId}/send-now`,
      {
        method: 'POST',
      }
    );
