...
interface ProjectDetailModalProps {
    ...
    onClose: () => void;
    onGenerateProposal: (projectId: string, templateRef?: string | null, priceLevel?: PriceLevel) => void;
}
                ...
                <div className={styles.dossierHeader} style={{ display: "block", position: "relative", paddingRight: "60px" }}>
                    ...
                    <div>
                        <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginBottom: "8px", alignItems: "center" }}>
                            {project.is_urgent && (
                                <span style={{ background: "#ef4444", color: "#fff", fontSize: "0.7rem", fontWeight: "bold", padding: "2px 8px", borderRadius: "4px", letterSpacing: "1px" }}>
                                    ⚡ URGENTE
                                </span>
                            )}
                            {project.is_featured && (
                                <span style={{ background: "#f59e0b", color: "#000", fontSize: "0.7rem", fontWeight: "bold", padding: "2px 8px", borderRadius: "4px", letterSpacing: "1px" }}>
                                    ⭐ DESTAQUE
                                </span>
                            )}
                            {project.client_plan && (
                                <span style={{ background: "rgba(99, 102, 241, 0.15)", color: "#818cf8", border: "1px solid rgba(99, 102, 241, 0.4)", fontSize: "0.7rem", fontWeight: "bold", padding: "2px 8px", borderRadius: "4px" }}>
                                    👑 {project.client_plan.toUpperCase()}
                                </span>
                            )}
                            {project.category && (
                                <span style={{ background: "rgba(255, 255, 255, 0.05)", color: "var(--color-text-muted)", fontSize: "0.7rem", padding: "2px 8px", borderRadius: "4px" }}>
                                    📁 {project.category}{project.subcategory ? ` / ${project.subcategory}` : ""}
                                </span>
                            )}
                        </div>
                        <h2 style={{ fontSize: "1.8rem", fontWeight: "bold", lineHeight: "1.2", marginBottom: "8px" }}>
                            {project.title}
                        </h2>
                        <div style={{ fontSize: "1.1rem", color: "#34d399", fontWeight: "600", display: "flex", alignItems: "center", gap: "8px" }}>
                            <span style={{ opacity: 0.7, fontSize: "0.9rem", fontWeight: "normal", color: "var(--color-text-muted)" }}>
                ...
                <div className={styles.dossierBody} style={{ padding: 0, overflow: "hidden" }}>
                    <div className={styles.dossierContentGrid}>
                        <div className={styles.dossierMain} style={{ padding: "2rem" }}>
                            ...
                            <div className={styles.decryptText}>
                                {project.description?.trim() ? (
                                    ...
                                    <div style={{ color: "var(--color-text-muted)", fontStyle: "italic", padding: "1.2rem", background: "rgba(255, 255, 255, 0.02)", borderRadius: "8px", border: "1px dashed rgba(255, 255, 255, 0.1)" }}>
                                        <span style={{ display: "block", color: "var(--color-primary)", fontWeight: "bold", marginBottom: "6px" }}>ℹ️ NENHUM TEXTO LIVRE INFORMADO</span>
                                        O contratante não incluiu uma descrição textual avulsa. Verifique o <strong>Arsenal (Skills)</strong> e o <strong>Briefing Estruturado</strong> no painel ao lado para as especificações técnicas da oportunidade.
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className={styles.dossierSidebar}>
                                ...
                                <div className={styles.techStack} style={{ flexWrap: "wrap" }}>
                                    {project.skills?.length > 0 ? (
                                        project.skills.map((skill) => (
                                            <span key={skill} className={styles.techTag} style={{ marginBottom: "6px", fontSize: "0.75rem" }}>
                                                {skill}
                                            </span>
                                        ))
                                    ) : (
                            ...
                            <div>
                                ...
                                <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                                    <div className="card p-3 bg-glass" style={{ border: "1px solid rgba(255,255,255,0.05)" }}>
                                        ...
                                        <span style={{ fontFamily: "monospace", fontWeight: "bold" }}>{project.posted_at || project.published_at || "Recente"}</span>
                                    </div>
                                    <div className="card p-3 bg-glass" style={{ border: "1px solid rgba(255,255,255,0.05)" }}>
                                        <span style={{ display: "block", fontSize: "0.7rem", color: "#64748b", marginBottom: "4px" }}>CONCORRÊNCIA</span>
                                        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                                            <span style={{ fontFamily: "monospace", fontWeight: "bold" }}>{project.proposals_count ?? 0} Candidatos</span>
                                    ...
                                    <div className="card p-3 bg-glass" style={{ border: "1px solid rgba(255,255,255,0.05)" }}>
                                        ...
                                        {project.client_plan && (
                                            <small style={{ display: "block", marginTop: "2px", color: "#818cf8", fontWeight: "600" }}>
                                    ...
                                    {project.last_client_activity && (
                                        <div className="card p-3 bg-glass" style={{ border: "1px solid rgba(255,255,255,0.05)" }}>
                                            <span style={{ display: "block", fontSize: "0.7rem", color: "#64748b", marginBottom: "4px" }}>ÚLTIMA ATIVIDADE DO CLIENTE</span>
                                    ...
                                    <div className="card p-3 bg-glass" style={{ border: "1px solid rgba(255,255,255,0.05)" }}>
                                        ...
                                        <span style={{ fontFamily: "monospace", fontWeight: "bold" }}>
                                            {project.payment_verified ? "✓ Pagamento verificado" : "Pagamento não verificado"}
                                            {project.client_rating != null ? ` · ★ ${project.client_rating.toFixed(1)}` : ""}
                                        </span>
                                    </div>
                                    {(project.project_type || project.deadline || project.contract_type) && (
                                        <div className="card p-3 bg-glass" style={{ border: "1px solid rgba(255,255,255,0.05)" }}>
                                            ...
                                            <span style={{ fontFamily: "monospace", fontWeight: "bold" }}>
                                                {project.contract_type ? (CONTRACT_LABELS[project.contract_type] || project.contract_type) : (project.project_type === "hourly" ? "Por hora" : "Preço fixo")}
                                                {project.deadline ? ` · ${project.deadline}` : ""}
                                            </span>
                                        </div>
                                    )}
                                    {(project.client_projects_posted != null || project.client_projects_paid != null) && (
                                        <div className="card p-3 bg-glass" style={{ border: "1px solid rgba(255,255,255,0.05)" }}>
                                            <span style={{ display: "block", fontSize: "0.7rem", color: "#64748b", marginBottom: "4px" }}>HISTÓRICO DO CLIENTE</span>
                                            <span style={{ fontFamily: "monospace", fontWeight: "bold" }}>
                                                {project.client_projects_posted ?? 0} publicados · {project.client_projects_paid ?? 0} pagos
                                            </span>
                                            {project.client_member_since && <small style={{ display: "block" }}>Desde {project.client_member_since}</small>}
                                        </div>
                                    )}
                                            ...
                                            {Object.entries(project.details)
                                                ...
                                                .map(([key, value]) => (
                                                    <small key={key} style={{ display: "block", marginBottom: "2px" }}>
                                                        <strong style={{ color: "var(--color-primary)" }}>{key.replace(/_/g, " ")}:</strong> {value}
                                                    </small>
                                                ))}
                                        </div>
                                    )}
                            ...
                            <div className="card p-3 bg-glass" style={{ border: "1px solid rgba(99, 102, 241, 0.3)", backgroundColor: "rgba(99, 102, 241, 0.05)", borderRadius: "10px" }}>
                                <h4 style={{ color: "var(--color-primary-light)", fontSize: "0.8rem", marginBottom: "0.6rem", textTransform: "uppercase", letterSpacing: "1px", display: "flex", alignItems: "center", gap: "6px" }}>
                            ...
                            <div style={{ marginTop: "auto", display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                                <button
                                    ...
                                    style={{ height: "44px", display: "flex", alignItems: "center", justifyContent: "center", gap: "8px", fontWeight: 700 }}
                                >
